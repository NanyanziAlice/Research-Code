...
egGlass     = computeFeatureVecNorm(s,'glass/obj18__','.png',90,4,3); %compute feature vector for each of the eight objects
egToy       = computeFeatureVecNorm(s,'toys/obj48__','.png',55,4,4); 
egBowl      = computeFeatureVecNorm(s,'bowls/obj25__','.png',150,5,4);
egCup       = computeFeatureVecNorm(s,'cups/obj11__','.png',40, 8,7);
egBottle    = computeFeatureVecNorm(s,'bottles/obj50__','.png',40,3,3);
egContainer = computeFeatureVecNorm(s,'container/obj70__','.png',70, 3, 5);
egBird      = computeFeatureVecNorm(s,'birds/obj74__','.png',60, 4, 4);
egIcecream  = computeFeatureVecNorm(s,'icecream/obj72__','.png',80, 4,3);
...
% figure(1)
% scatter3(NewGlass(v2,:),NewGlass(v3,:),NewGlass(v1,:),'o','MarkerEdgeColor',[0.2 .65 .5]); 
% % view(-30,20);
% hold on
% scatter3(NewToy(v2,:),NewToy(v3,:),NewToy(v1,:),'+','g');
% hold on
% scatter3(NewBowl(v2,:),NewBowl(v3,:),NewBowl(v1,:),'*','b');
% hold on
% scatter3(NewCup(v2,:),NewCup(v3,:),NewCup(v1,:),'s','c');
% hold on
% scatter3(NewBottle(v2,:),NewBottle(v3,:),NewBottle(v1,:),'d','m');
% hold on
% scatter3(NewCont(v2,:),NewCont(v3,:),NewCont(v1,:),'V','y');
% hold on
% scatter3(NewBird(v2,:),NewBird(v3,:),NewBird(v1,:),'*','r');
% hold on
% scatter3(NewIcecream(v2,:),NewIcecream(v3,:),NewIcecream(v1,:),'^','k');
% grid off
