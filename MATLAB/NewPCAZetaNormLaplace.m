%PCA analysis based on Zeta function of Normalised Laplacian based on
%Laplace

%compute feature vector of laplacian eigenvalues for the objects

% arguments to the computeFeatureVectorLaplaceMellin() function which 
...
s = 2;                     %set value of Laplace exponent lambda
p = [1,2,3,4];             %size of the leading eigenvalues

egGlass     = computeFeatureVectorZetaLaplace(s,p,'glass/obj18__','.png',90,4,3); 
egToy       = computeFeatureVectorZetaLaplace(s,p,'toys/obj48__','.png',55,4,4); 
egBowl      = computeFeatureVectorZetaLaplace(s,p,'bowls/obj25__','.png',150,5,4);
egCup       = computeFeatureVectorZetaLaplace(s,p,'cups/obj11__','.png',40, 8,7);
egBottle    = computeFeatureVectorZetaLaplace(s,p,'bottles/obj50__','.png',40,3,3);
egContainer = computeFeatureVectorZetaLaplace(s,p,'container/obj70__','.png',70, 3, 5);
egBird      = computeFeatureVectorZetaLaplace(s,p,'birds/obj74__','.png',60, 4, 4);
egIcecream  = computeFeatureVectorZetaLaplace(s,p,'icecream/obj72__','.png',80, 4,3);
...
St = S';                              % transpose matrix so that obsercvations ae rows a dimensions are cccolumns
